...
```
var mssqlConnector = require('volos-mssql');
var http = require('http');

var profile = {
  user: 'volos',
  password: 'volos',
  server: "nsa.rds.amazon.com",
  database: "volos"
};

var mssqlConnectorObject = new mssqlConnector.mssqlConnector({"profile": profile});

var svr = http.createServer(function (req, resp) {
  mssqlConnectorObject.dispatchRequest(req, resp);
});

svr.listen(9089, function () {
  console.log('volos-mssql node server is listening');
});
...
vault.get('my_profile_key', function (profileString) {
  if (!profileString) {
    ...
    svr.listen(9089, function () {
      mssqlConnectorObject = new mssqlConnector.mssqlConnector({"profile": profile});
      console.log('volos-mssql node server is listening');
    });
  }
});
...
* **server** - The database host IP address. For example: ``nsa.rds.amazonaws.com``
* **database** - The name of the database you wish to query.
...
Note that these are the same keys that are required in the plaintext version of the profile.  If this command completes successfully you will find two new files: `store.js` and `keys.js`. Place them in the root directory of the ``volos-mssql`` module. 
