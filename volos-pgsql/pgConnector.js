...
var PgConnector = function (options) {
  ...
  this.getQueryData = function (req, resp, setupResult) {
    ...
    return(this.performQuery(req, resp, queryString, limit, setupResult.client, setupResult.done, options.rowCallback, options.rowCallbackContext));
  }
    ...
    this.prepareRequest(req, resp).then(
        function (requestInfo) {
          if (!requestInfo.found) {
            ...
            var conString = 'postgres://' + profile.username + ':' + profile.password + '@' + profile.host + (profile.port ? (':' + profile.port) : '') + '/' + profile.database;
